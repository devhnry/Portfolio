		...
		return (
				<header className={`font-inter capitalize text-md text-paragraph-1 md:text-[1.0625rem]`}>
						...
						<img className={`size-[10px]`} src={"./src/assets/copyright.svg"} alt={``}/>
										...
										<p className={`font-light text-paragraph-1 text-sm xsl:text-[1.05rem] md:text-[clamp(0.9375rem,2.1vw,1.125rem)]`}>(Software
																																																																			Engineer)</p>
								</div>
														...
														<li key={item} className="relative overflow-hidden flex cursor-none">
																<div className="block h-[28px] overflow-hidden"
																		...
																		>
																				<span>{item}</span>
																				<m.span initial={{y: '0%'}}
																								animate={{opacity: hoveredIndex === i ? 1 : 0.5}}>{item}</m.span>
																		</m.div>
																</div>
																<span>,</span>
														</li>
