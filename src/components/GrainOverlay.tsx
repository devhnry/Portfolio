import { useEffect, useState } from "react"

export function GrainOverlay() {
	const [isLoaded, setIsLoaded] = useState(false)

	useEffect(() => {
		setIsLoaded(true)
	}, [])

	return (
		<div
			className={`pointer-events-none fixed inset-0 z-50 mix-blend-multiply
        [background-image:url('data:image/svg+xml;base64,')]
        after:content-[''] after:absolute after:inset-0 after:bg-repeat after:opacity-30 after:mix-blend-overlay
        before:content-[''] before:absolute before:inset-0 before:bg-[#0f0f0f] before:opacity-[0.2] before:mix-blend-darken
        ${isLoaded ? "opacity-100" : "opacity-0"}
        transition-opacity duration-1000`}
			aria-hidden="true"
		/>
	)
}
